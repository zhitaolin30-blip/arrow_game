...
使用 Python 和 Pygame 实现的点击式箭头解谜游戏。项目按照软件工程第二次个人作业要求完成，包含三个基础关卡和一个高难度扩展关、动画反馈、失误机制、自动化测试、自检脚本和博客初稿。
...
- 三个基础关棋盘依次为 3×3、4×4、5×5，共 9、16、25 格；高难度扩展关为 6×6、36 格，包含 14 个箭头且开局只有两个合法选择。
...
截图脚本会在 `screenshots/` 中生成开始、游戏、碰撞、通关、失败和扩展关六种真实界面截图。
...
项目还包含一个 DFS 求解器。测试会对四个关卡分别求解并逐步回放，确保所有内置关卡均能通关。扩展关包含 14 个箭头，开局仅有两个合法选择，其中多步只有唯一解，难度明显高于前三关。
